 ...
 */
	...
	if( defined('WCCM_VERISON') ) {
		...
		//
		$shop_isle_news_hide = get_theme_mod('shop_isle_news_hide');
		...
		if ( isset($shop_isle_news_hide) && $shop_isle_news_hide != 1 ) {
			...
			$args = array(
				...
				echo '<section class="module-small module-news">';
					echo '<div class="container">';
			$news_query = new WP_Query( $args );
			while ( $news_query-> have_posts() ) : $news_query->the_post();
				
							echo '<div class="col-sm-6 front-page-news-post">';
								echo '<a href="';
								echo the_permalink();
								echo '"><h2>';
								echo the_title().'</h2></a>';
		...
		$shop_isle_about_hide = get_theme_mod('shop_isle_about_hide');
		
		if ( isset($shop_isle_about_hide) && $shop_isle_about_hide != 1 ) {
			echo '<section class="module-small module-about">';
						echo '<div class="container">';
							echo '<div class="col-sm-7">';
								echo '<div class="google-maps"><iframe style="border: 0;" src="';
								echo get_theme_mod( 'shop_isle_about_google_map');
								echo '" width="600" height="500" frameborder="0" allowfullscreen="allowfullscreen"></iframe></div>';
							echo '</div>';
							echo '<div class="col-sm-5" style="font-size:1.1em;margin-top:0;">';
								if(have_posts()) :
		...
		$shop_isle_big_banners_hide = get_theme_mod('shop_isle_big_banners_hide');
		...
		if( ( isset($shop_isle_big_banners_hide) && $shop_isle_big_banners_hide != 1) || is_customize_preview() ) {
			...
			if(current_user_can( 'edit_theme_options')) {
				$shop_isle_big_banners = get_theme_mod( 'shop_isle_big_banners', json_encode( array(
				
					array(
						'image_url' => get_template_directory_uri() . '/assets/images/bigbanner.jpg',
						'link'      => '#'
					),
				) ) );
			} else {
				$shop_isle_big_banners = get_theme_mod( 'shop_isle_big_banners' );
			}
			
			
			
			if( !empty( $shop_isle_big_banners ) ) {
								...
								foreach($shop_isle_big_banners_decoded as $shop_isle_big_banner) {
									...
									if ( !empty($shop_isle_big_banner->image_url) ) {
										...
										if ( !empty($shop_isle_big_banner->link) ) {
											...
											if (function_exists ( 'icl_t' ) && !empty($shop_isle_big_banner->id)){
												$shop_isle_big_banner_link = icl_t( 'Banner '.$shop_isle_big_banner->id, 'Banner link', $shop_isle_big_banner->link );
												$shop_isle_big_banner_image_url = icl_t( 'Banner '.$shop_isle_big_banner->id, 'Banner image', $shop_isle_big_banner->image_url );
												echo '<a href="' . esc_url( $shop_isle_big_banner_link ) . '"><img src="' . esc_url( $shop_isle_big_banner_image_url ) . '"></a>';
											} else {					
												echo '<a href="' . esc_url( $shop_isle_big_banner->link ) . '"><img src="' . esc_url( $shop_isle_big_banner->image_url ) . '"></a>';
											}
										}
										else {
											if (function_exists ( 'icl_t' ) && !empty($shop_isle_big_banner->id)){
												$shop_isle_big_banner_image_url = icl_t( 'Banner '.$shop_isle_big_banner->id, 'Banner image', $shop_isle_big_banner->image_url );
												echo '<a><img src="' . esc_url( $shop_isle_big_banner_image_url ) . '"></a>';
											} else {
